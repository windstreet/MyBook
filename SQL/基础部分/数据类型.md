...
##### 6.4、复合类型数据修改
修改复合类型数据时，可以整体更新复合类型字段，也可更新复合类型字段的子字段。

示例如下：
```sql
UPDATE students SET lesson = ROW('Liuht', 6) WHERE name = 'liuht';      --整字段更新
--  UPDATE 1


UPDATE students SET lesson.times = 7 WHERE name = 'itbilu';     --子字段更新                                                                                      
-- UPDATE 1
